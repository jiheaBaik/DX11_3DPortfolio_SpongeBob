...
HRESULT CLight::Render(CShader * pShader, CVIBuffer_Rect * pVIBuffer)
{
	...
	{
		if (FAILED(pShader->Set_RawValue("g_vLightDir", &m_LightDesc.vDirection, sizeof(_float3))))
			return E_FAIL;

		iPassIndex = 1;

	}
	else if (LIGHTDESC::TYPE_POINT == m_LightDesc.eLightType)
	{
		if (FAILED(pShader->Set_RawValue("g_vLightPos", &m_LightDesc.vPosition, sizeof(_float3))))
			return E_FAIL;
