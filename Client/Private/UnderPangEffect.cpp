#include "stdafx.h"
#include "..\Public\UnderPangEffect.h"
#include "GameInstance.h"
#include<iostream>

CUnderPangEffect::CUnderPangEffect(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CUnderPangEffect::CUnderPangEffect(const CUnderPangEffect & Prototype)
	: CGameObject(Prototype)
{
}

HRESULT CUnderPangEffect::NativeConstruct_Prototype()
{
	return S_OK;
}

HRESULT CUnderPangEffect::NativeConstruct(void * pArg)
{	
	...
	if (static_cast<_int*>(pArg) != nullptr)
		look = *(_int*)pArg;

	if (FAILED(SetUp_Components()))
		return E_FAIL;
	//m_pTransformCom->Rotation(XMLoadFloat3(&_float3(1, 0, 0)), -1);

	return S_OK;
}

void CUnderPangEffect::Tick(_double TimeDelta)
{

	if (m_IsRender)
	{
		//m_fFrame += 6 * TimeDelta;
		m_fFrame += 1 * 0.2f;
	...
	////(pos : look)
	//_vector		vSandyLook = pSnadyTransform->Get_State(CTransform::STATE_RIGHT) * look;// *-1;
	...
	//else
	//	pos = XMVectorSet(fSnadyPos.x + (fSandyLook.x * fdistance), fSnadyPos.y + (fSandyLook.y * fdistance) + 5, fSnadyPos.z + fSandyLook.z + (fSandyLook.z * fdistance), 1);


	m_pTransformCom->Set_State(CTransform::STATE_POSITION, vSnadyPos + XMVectorSet(0,1,0,1));
	...
	m_pTransformCom->Set_Scaled(_float3(20,20,0));
...
void CUnderPangEffect::LateTick(_double TimeDelta)
{
...
HRESULT CUnderPangEffect::Render()
{
...
HRESULT CUnderPangEffect::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Texture */
	if (FAILED(__super::Add_Component(TEXT("Com_Texture"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_UnderPang"), (CComponent**)&m_pTextureCom)))
		return E_FAIL;
...
HRESULT CUnderPangEffect::SetUp_SRV()
{
...
CUnderPangEffect * CUnderPangEffect::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CUnderPangEffect*		pInstance = new CUnderPangEffect(pDevice, pContext);
...
CGameObject * CUnderPangEffect::Clone(void * pArg)
{
	CUnderPangEffect*		pInstance = new CUnderPangEffect(*this);
