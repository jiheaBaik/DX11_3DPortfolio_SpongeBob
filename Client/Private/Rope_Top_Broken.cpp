#include "stdafx.h"
#include "..\Public\Rope_Top_Broekn.h"
#include "GameInstance.h"



CRope_Top_Broekn::CRope_Top_Broekn(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CRope_Top_Broekn::CRope_Top_Broekn(const CRope_Top_Broekn & Prototype)
	: CGameObject(Prototype)
{
}

HRESULT CRope_Top_Broekn::NativeConstruct_Prototype()
{
	return S_OK;
}

HRESULT CRope_Top_Broekn::NativeConstruct(void * pArg)
{
	...
	m_pTransformCom->Rotation(XMLoadFloat3(&_float3(0.f, 1.f, 0.f)), 0.4);
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(28.04, 0.f, 52.69,1 ));
...
void CRope_Top_Broekn::Tick(_double TimeDelta)
{
	////Collider
...
void CRope_Top_Broekn::LateTick(_double TimeDelta)
{
...
HRESULT CRope_Top_Broekn::Render()
{
...
HRESULT CRope_Top_Broekn::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_VIBuffer */
	if (FAILED(__super::Add_Component(TEXT("Com_VIBuffer"), LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_Rope_Top_Broken"), (CComponent**)&m_pVIBufferCom)))
		return E_FAIL;
...
HRESULT CRope_Top_Broekn::SetUp_ConstantTable()
{
...
CRope_Top_Broekn * CRope_Top_Broekn::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CRope_Top_Broekn*		pInstance = new CRope_Top_Broekn(pDevice, pContext);
...
CGameObject * CRope_Top_Broekn::Clone(void * pArg)
{
	CRope_Top_Broekn*		pInstance = new CRope_Top_Broekn(*this);
