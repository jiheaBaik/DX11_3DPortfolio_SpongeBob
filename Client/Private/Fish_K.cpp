#include "stdafx.h"
#include "..\Public\Fish_K.h"


#include "GameInstance.h"

CFish_K::CFish_K(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CFish_K::CFish_K(const CFish_K & Prototype)
	: CGameObject(Prototype)
{
}

HRESULT CFish_K::NativeConstruct_Prototype()
{
	return S_OK;
}

HRESULT CFish_K::NativeConstruct(void * pArg)
{
...
void CFish_K::Tick(_double TimeDelta)
{
	m_pVIBufferCom->Update_Animation(TimeDelta);

}

void CFish_K::LateTick(_double TimeDelta)
{
...
HRESULT CFish_K::SetUp_Components()
{
	...
	/* For.Com_VIBuffer */
	if (FAILED(__super::Add_Component(TEXT("Com_VIBuffer"), LEVEL_GAMEPLAY, TEXT("Prototype_Component_Model_Fish_K"), (CComponent**)&m_pVIBufferCom)))
		return E_FAIL;
...
HRESULT CFish_K::SetUp_ConstantTable()
{
...
CFish_K * CFish_K::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CFish_K*		pInstance = new CFish_K(pDevice, pContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX("Failed to Created : CFish_K");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CFish_K::Clone(void * pArg)
{
	CFish_K*		pInstance = new CFish_K(*this);

	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSG_BOX("Failed to Cloned : CFish_K");
		Safe_Release(pInstance);
	}
	return pInstance;
}
