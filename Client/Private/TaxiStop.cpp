#include "stdafx.h"
#include "..\Public\TaxiStop.h"



#include "GameInstance.h"

CTaxiStop::CTaxiStop(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CTaxiStop::CTaxiStop(const CTaxiStop & Prototype)
	: CGameObject(Prototype)
{
}

HRESULT CTaxiStop::NativeConstruct_Prototype()
{
	return S_OK;
}

HRESULT CTaxiStop::NativeConstruct(void * pArg)
{
...
void CTaxiStop::Tick(_double TimeDelta)
{
	

}

void CTaxiStop::LateTick(_double TimeDelta)
{
...
HRESULT CTaxiStop::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_VIBuffer */
	if (FAILED(__super::Add_Component(TEXT("Com_VIBuffer"), LEVEL_FIELD, TEXT("Prototype_Component_Model_TaxiStop"), (CComponent**)&m_pVIBufferCom)))
		return E_FAIL;
...
HRESULT CTaxiStop::SetUp_ConstantTable()
{
...
CTaxiStop * CTaxiStop::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CTaxiStop*		pInstance = new CTaxiStop(pDevice, pContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX("Failed to Created : CTaxiStop");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CTaxiStop::Clone(void * pArg)
{
	CTaxiStop*		pInstance = new CTaxiStop(*this);

	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSG_BOX("Failed to Cloned : CTaxiStop");
		Safe_Release(pInstance);
	}
	return pInstance;
}
