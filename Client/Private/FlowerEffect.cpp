#include "stdafx.h"
#include "..\Public\FlowerEffect.h"
#include "GameInstance.h"
#include<iostream>

CFlowerEffect::CFlowerEffect(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CFlowerEffect::CFlowerEffect(const CFlowerEffect & Prototype)
	: CGameObject(Prototype)
{
}

HRESULT CFlowerEffect::NativeConstruct_Prototype()
{
	return S_OK;
}

HRESULT CFlowerEffect::NativeConstruct(void * pArg)
{	
...
void CFlowerEffect::Tick(_double TimeDelta)
{


	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);

		CTransform* pTaxiStopTransform = (CTransform*)pGameInstance->Get_Component(LEVEL_FIELD, TEXT("Layer_TaxiStop"), TEXT("Com_Transform"));
		_vector		vTaxiStopPos = pTaxiStopTransform->Get_State(CTransform::STATE_POSITION);

		m_pTransformCom->Set_State(CTransform::STATE_POSITION, vTaxiStopPos + XMVectorSet(0, 3.5, 0, 1));
	...
	m_pTransformCom->Set_Scaled(_float3(4.6,1.6,0));


	RELEASE_INSTANCE(CGameInstance);
}

void CFlowerEffect::LateTick(_double TimeDelta)
{
...
HRESULT CFlowerEffect::SetUp_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Texture */
	if (FAILED(__super::Add_Component(TEXT("Com_Texture"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Flower02"), (CComponent**)&m_pTextureCom)))
		return E_FAIL;
...
HRESULT CFlowerEffect::SetUp_SRV()
{
...
CFlowerEffect * CFlowerEffect::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CFlowerEffect*		pInstance = new CFlowerEffect(pDevice, pContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX("Failed to Created : CFlowerEffect");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CFlowerEffect::Clone(void * pArg)
{
	CFlowerEffect*		pInstance = new CFlowerEffect(*this);

	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSG_BOX("Failed to Cloned : CFlowerEffect");
		Safe_Release(pInstance);
	}
	return pInstance;
}
