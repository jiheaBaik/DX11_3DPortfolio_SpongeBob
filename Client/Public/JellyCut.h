...
BEGIN(Client)
class CJellyCut final : public CGameObject
{
...
private:
	CJellyCut(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CJellyCut(const CJellyCut& Prototype);
	virtual ~CJellyCut() = default;
...
public:
	...
	void Set_AlPha() { m_IsAlphaSet = true; }
...
public:
	static CJellyCut* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
