...
BEGIN(Client)
class CJellyGround final : public CGameObject
{
...
public:
	CJellyGround(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CJellyGround(const CJellyGround& CRing_Top);
	virtual ~CJellyGround() = default;
...
private:
	OBJINFODESC m_Info;
public:
	static CJellyGround* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
