#include "..\Public\Robo3.h"


#include "GameInstance.h"

CRobo3::CRobo3(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObject(pDevice, pContext)
{
}

CRobo3::CRobo3(const CRobo3 & Prototype)
	: CGameObject(Prototype)
{
}

HRESULT CRobo3::NativeConstruct_Prototype()
{
	return S_OK;
}

HRESULT CRobo3::NativeConstruct(void * pArg)
{
...
void CRobo3::Tick(_double TimeDelta)
{
	m_pVIBufferCom->Update_Animation(TimeDelta);

}

void CRobo3::LateTick(_double TimeDelta)
{
...
HRESULT CRobo3::SetUp_Components()
{
	...
	/* For.Com_VIBuffer */
	if (FAILED(__super::Add_Component(TEXT("Com_VIBuffer"), LEVEL_STATIC, TEXT("Prototype_Component_Model_Robo3"), (CComponent**)&m_pVIBufferCom)))
		return E_FAIL;
...
HRESULT CRobo3::SetUp_ConstantTable()
{
...
CRobo3 * CRobo3::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CRobo3*		pInstance = new CRobo3(pDevice, pContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX("Failed to Created : CRobo3");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CRobo3::Clone(void * pArg)
{
	CRobo3*		pInstance = new CRobo3(*this);

	if (FAILED(pInstance->NativeConstruct(pArg)))
	{
		MSG_BOX("Failed to Cloned : CRobo3");
		Safe_Release(pInstance);
	}
	return pInstance;
}
